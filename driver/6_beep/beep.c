#include <linux/types.h>
#include <linux/kernel.h>
#include <linux/delay.h>
#include <linux/ide.h>
#include <linux/init.h>
#include <linux/module.h>
#include <linux/errno.h>
#include <linux/gpio.h>
#include <linux/cdev.h>
#include <linux/device.h>
#include <linux/of.h>
#include <linux/of_address.h>
#include <linux/of_gpio.h>
#include <asm/mach/map.h>
#include <asm/uaccess.h>
#include <asm/io.h>
/***************************************************************
文件名 : beep.c
作者 : lanyuana
版本 : V1.0
描述 : 蜂鸣器。
其他 : 无
日志 : 初版 V1.0 2025/11/26 lanyuana创建
***************************************************************/

#define BEEP_CNT   1               /*设备号个数*/
#define BEEP_NAME  "beep"    /*设备名*/

#define BEEPOFF 0/*关闭蜂鸣器*/
#define BEEPON 1/*打开蜂鸣器*/

/*beep设备结构体*/
struct beep_dev
{
    dev_t devid;    /*设备号*/
    struct cdev cdev;   /*cdev*/
    struct class *class;    /*类*/
    struct device *device;  /*设备*/
    int major;  /*主设备号*/    
    int minor;  /*次设备号*/
    struct device_node *nd; /*设备节点*/
    int beep_gpio;/*beep所使用的GPIO编号*/
};

struct beep_dev beep;/*beep设备*/

/*
* @description : beep打开/关闭
* @param – inode : 传递给驱动的 inode
* @param - filp : 设备文件，file 结构体有个叫做 private_data 的成员变量
*                 一般在 open 的时候将 private_data 指向设备结构体。
* @return : 0 成功;其他 失败
*/
void beep_switch(struct beep_dev *dev,u8 sta)
{   //beep是低电平触发
    if(sta == BEEPON)
    {
        gpio_set_value(dev->beep_gpio,0);/*打开beep*/
    }
    else if(sta == BEEPOFF)
    {
        gpio_set_value(dev->beep_gpio,1); /*关闭beep*/
    }
}

/*
* @description : 打开设备
* @param – inode : 传递给驱动的 inode
* @param - filp : 设备文件，file 结构体有个叫做 private_data 的成员变量
*                 一般在 open 的时候将 private_data 指向设备结构体。
* @return : 0 成功;其他 失败
*/
static int beep_open(struct inode *inode,struct file *filp)
{
    /*用户实现具体功能*/
    filp->private_data = &beep;/*设置私有数据*/
    return 0;
}

/*
* @description : 从设备读取数据
* @param - filp : 要打开的设备文件(文件描述符)
* @param - buf : 返回给用户空间的数据缓冲区
* @param - cnt : 要读取的数据长度
* @param - offt : 相对于文件首地址的偏移
* @return : 读取的字节数，如果为负值，表示读取失败
*/
static ssize_t beep_read(struct file *filp,char __user *buf,size_t cnt,loff_t *offt)
{
    int retvalue=0;
    int val =0;
    unsigned char status =0;//存放发送给用户的数据
    struct beep_dev *dev = filp->private_data;
    val = gpio_get_value(dev->beep_gpio);//读取值
    printk("GPIO %d value: %d\r\n", dev->beep_gpio, val);
    if(val != 0)//高电平是关闭
    {
        status = BEEPOFF;
    }
    else 
    {
        status = BEEPON;
    }
    retvalue = copy_to_user(buf,&status,1);
    if(retvalue)
    {
        printk("send to user failed!\r\n");
        return -EFAULT;
    }
    //read成功通常返回读取的字节数
    return 1;
}

/*
* @description : 向设备写数据
* @param - filp : 设备文件，表示打开的文件描述符
* @param - buf : 要写给设备写入的数据
* @param - cnt : 要写入的数据长度
* @param - offt : 相对于文件首地址的偏移
* @return : 写入的字节数，如果为负值，表示写入失败
*/
static ssize_t beep_write(struct file *filp,const char __user *buf,size_t cnt,loff_t *offt)
{
    /*用户实现具体功能*/
    int retvalue =0;
    unsigned char databuf[1];
    unsigned char beeptat;
    struct beep_dev *dev = filp->private_data;
    int val = 0;
    retvalue = copy_from_user(databuf,buf,cnt);//从传入的buf写入到databuf 个数为cnt
    if(retvalue)
    {
        printk("kernal recevdata failed!\r\n");
        return -EFAULT;
    }
    beeptat = databuf[0];/*从用户输入的指令中获得控制值 获取状态值*/
    if(beeptat == BEEPON)
    {
        gpio_set_value(dev->beep_gpio,0);/*打开beep*/
        val = gpio_get_value(dev->beep_gpio);//读取值
        printk("val = %d",val);
    }
    else if(beeptat == BEEPOFF)
    {
        gpio_set_value(dev->beep_gpio,1);/*关闭beep*/
        val = gpio_get_value(dev->beep_gpio);//读取值
        printk("val = %d",val);
    }
    return 0;
}


/*
* @description : 关闭/释放设备
* @param - filp : 要关闭的设备文件(文件描述符)
* @return : 0 成功;其他 失败
*/
static int beep_release(struct inode *inode,struct file *filp)
{
    /*用户实现具体功能*/
    return 0;
}

/*
*   设备操作函数结构体
*/
static struct  file_operations beep_fops ={
    .owner = THIS_MODULE,
    .open = beep_open,
    .read = beep_read,
    .write = beep_write,
    .release = beep_release,
};

/*
* @description : 驱动入口函数
* @param：       无
* @return : 0 成功;其他 失败
*/
static int __init beep_init(void)
{
    /*入口函数具体内容*/
    int ret;
    /*设置beep所使用的GPIO*/
    /*1.获取设备树中属性数据*/
    beep.nd = of_find_node_by_path("/beep");
    if(beep.nd == NULL)
    {
        printk("beep node cant not found!\r\n");
        return -EINVAL;
    }
    else
    {
        printk("beep node has been found!\r\n");
    }
    /*获取设备树中GPIO属性 得到beep所使用的beep编号*/
    beep.beep_gpio = of_get_named_gpio(beep.nd,"beep-gpios",0);
    if(beep.beep_gpio < 0)
    {
        printk("beep.beep_gpio cant not found!\r\n");
        return -EINVAL;        
    }
    else
    {
        printk("beep.beep_gpio has been found!\r\n");
    }
    /*设置GPIO5_IO01为输出 输出高电平 默认关闭beep*/
    ret = gpio_direction_output(beep.beep_gpio,1); // 初始高=关
    if(ret < 0)
    {
        printk("gpio_direction_output faibeep!\r\n");
        return -EINVAL;        
    }

    /*注册字符设备驱动*/
    if(beep.major)//指定了major
    {
        beep.devid = MKDEV(beep.major,0);//次设备号选择0
        register_chrdev_region(beep.devid,BEEP_CNT,BEEP_NAME);
    }
    else
    {
        alloc_chrdev_region(&beep.devid,0,BEEP_CNT,BEEP_NAME);//申请设备号
        beep.major = MAJOR(beep.devid);//获得主设备号
        beep.minor = MINOR(beep.devid);//获得次设备号
    }
    
    beep.cdev.owner = THIS_MODULE;//设置cdev.owner属性
    cdev_init(&beep.cdev,&beep_fops);//初始化cdev

    cdev_add(&beep.cdev,beep.devid,BEEP_CNT);//增加一个cdev

    /*创建类*/
    beep.class = class_create(THIS_MODULE,BEEP_NAME);
    /*创建设备*/
    beep.device = device_create(beep.class,NULL,beep.devid,NULL,BEEP_NAME);

    return 0;
}

/*
* @description : 驱动出口函数
* @param：       无
* @return : 0 成功;其他 失败
*/
static void __exit beep_exit(void)
{
    /*注销字符设备*/
    cdev_del(&beep.cdev);
    unregister_chrdev_region(beep.devid,BEEP_CNT);
    device_destroy(beep.class,beep.devid);
    class_destroy(beep.class);
    printk("chrdevbase_exit()\r\n");
}

/*
*将上述函数指定为驱动的入口和出口函数
*/
module_init(beep_init);
module_exit(beep_exit);
/*
*LICENSE和作者信息
*/
MODULE_LICENSE("GPL");
MODULE_AUTHOR("LanYuanA");