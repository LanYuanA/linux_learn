...
void timer_function(unsigned long arg)
{
    ...
    if(atomic_read(&dev->releasekey))
    {
        /*完成一次按键操作*/
        if(dev->async_queue)
            kill_fasync(&dev->async_queue,SIGIO,POLL_IN);//发送信号
    }   
...
static int imx6uirq_fasync(int fd,struct file *filp,int on)
{
    ...
    if(fasync_helper(fd,filp,on,&dev->async_queue)<0)//初始化结构体指针
        return -EIO;
    return 0;
}

static int imx6uirq_release(struct inode *inode, struct file *filp)//删除结构体指针
{
